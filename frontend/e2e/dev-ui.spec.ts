...
//   1) 컴포넌트 export 34개 중 갤러리가 선언한 32개 표본이 전부 마운트되었는가
//      (제외 2개 = CommandPalette · CommandHint — page.tsx 머리말에 이유를 적어 두었다)
...
test.describe("/dev/ui — shared/ui 격리 갤러리", () => {
  test("모든 표본이 마운트되고 uncaught error 가 없다", async ({ page }) => {
    ...
    await expect(page.locator(".devui-item")).toHaveCount(SPECIMENS.length);

    // 표본 이름이 선언과 정확히 일치(순서까지) — 이름 변경/누락 감지.
    const names = await page.locator(".devui-item .devui-item-name").allInnerTexts();
    expect(names.map((s) => s.trim())).toEqual(SPECIMENS);

    // 데이터 없이 렌더되는 화면이므로 백엔드 호출이 있어서는 안 된다.
    expect(uniq([...sink.api404, ...sink.apiOther4xx5xx]), "갤러리는 API 를 호출하지 않는다").toEqual([]);
  ...
  test("primitives 의 클래스 계약이 유지된다", async ({ page }) => {
    ...
    await expect(page.locator(".pv-page-header")).toHaveCount(1);
    await expect(page.locator(".pv-page-header .pv-page-title")).toHaveText("페이지 제목");
    await expect(page.locator(".pv-page-header .pv-page-subtitle")).toBeVisible();
    await expect(page.locator(".pv-page-header .pv-breadcrumb a")).toHaveCount(1);

    // PageContent + PageHeader 가 공통으로 쓰는 폭 컨테이너
    await expect(page.locator(".container-pv").first()).toBeVisible();
    ...
    await expect(page.locator(".pv-tabs")).toHaveCount(1);
    await expect(page.locator(".pv-tabs .pv-tab")).toHaveCount(3);
    await expect(page.locator(".pv-tabs .pv-tab.active")).toHaveCount(1);
    await expect(page.locator(".pv-tabs .pv-tab.active")).toHaveText("개요");
  ...
  test("Tabs 표본이 실제로 상호작용한다(active 가 옮겨감)", async ({ page }) => {
    ...
    const tabs = page.locator(".pv-tabs .pv-tab");
  ...
  test("States 의 .tstate-* 계약과 접근성 속성이 유지된다", async ({ page }) => {
    ...
    await expect(page.locator(".tstate")).toHaveCount(6);
    await expect(page.locator(".tstate.tstate-loading")).toHaveCount(2);
    await expect(page.locator(".tstate.tstate-empty")).toHaveCount(2);
    await expect(page.locator(".tstate.tstate-error")).toHaveCount(2);

    // 하위 요소 계약
    await expect(page.locator(".tstate-loading .tstate-spinner")).toHaveCount(2);
    await expect(page.locator(".tstate-empty .tstate-glyph")).toHaveCount(2);
    await expect(page.locator(".tstate-sub")).toHaveCount(3); // sub 를 넘긴 variant 만

    // 로딩은 role=status, 오류는 role=alert — 스크린리더 계약
    await expect(page.locator('.tstate-loading[role="status"]')).toHaveCount(2);
    await expect(page.locator('.tstate-error[role="alert"]')).toHaveCount(2);

    await expect(page.locator(".tstate-loading").first()).toContainText("[ LOADING ]");
    await expect(page.locator(".tstate-error").first()).toContainText("[ ERROR ]");
  });
  ...
  test("MiniViz / StatGrid / Stat / SectionHead 의 클래스 계약이 유지된다", async ({ page }) => {
    ...
    await expect(page.locator(".t-miniviz")).toHaveCount(5);

    // StatGrid 1개 안에 Stat 3개 + 단독 Stat 표본 2개 = .tstat 5개
    await expect(page.locator(".tstat-grid")).toHaveCount(1);
    await expect(page.locator(".tstat-grid .tstat")).toHaveCount(3);
    await expect(page.locator(".tstat")).toHaveCount(5);
    await expect(page.locator(".tstat .tstat-key").first()).toHaveText("CAGR");
    await expect(page.locator(".tstat .tstat-val").first()).toHaveText("14.2%");

    // SectionHead — index 있음/없음
    await expect(page.locator(".tpage-section-head")).toHaveCount(2);
    await expect(page.locator(".tpage-section-head .sh-label").first()).toHaveText("유동성 게이트");
    await expect(page.locator(".tpage-section-head .sh-index")).toHaveCount(1); // index 를 넘긴 쪽만
  });
  ...
  test("feedback 의 .skeleton / .sparkline 계약이 유지된다", async ({ page }) => {
    ...
    await expect(page.locator(".skeleton")).toHaveCount(24);

    // Sparkline: data 2개 이상인 것만 svg 를 그린다(1개짜리는 빈 span)
    await expect(page.locator(".sparkline")).toHaveCount(2);
    await expect(page.locator(".sparkline svg polyline")).toHaveCount(2);
  ...
  test("kit 은 클래스가 없으므로 role·태그 구조로 검증한다", async ({ page }) => {
    ...
    await expect(page.locator('[role="switch"]')).toHaveCount(5);
    await expect(page.locator('[role="switch"][aria-checked="true"]')).toHaveCount(5);
