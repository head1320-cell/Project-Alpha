"use client";
// ═══════════════════════════════════════════════════════════════════════════════
// insights/parts — 후보·프로덕션 공용 비주얼 (recharts + SVG). 라이트 터미널 토큰.
// ═══════════════════════════════════════════════════════════════════════════════
import React from "react";
import { AreaChart, Area, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, ReferenceLine } from "recharts";
import type { CompanyData, FactorVal, FactorGroup, ModelResult, Scenario, Tone } from "./types";
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 8, right: 10, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey="t" tick={{ fontSize: 10, fill: MUTED, fontFamily: "var(--t-mono)" }} interval={Math.max(0, Math.floor(data.length / 6))} tickLine={false} axisLine={{ stroke: BORDER }} />
...
export function Radar({ groups, size = 240 }: { groups: FactorGroup[]; size?: number }) {
  ...
  if (n < 3) return null;
  const avg = groups.map((g) => g.factors.reduce((s, x) => s + x.pct, 0) / (g.factors.length || 1));
