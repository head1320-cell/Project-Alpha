...
#:
#: ★왜 고치지 않고 기준선으로 잠그는가 — 근거가 처음 설명과 다르다(실측 후 정정)★
#: `.card-md` 는 globals.css 에 **두 번** 정의돼 있다 — 191행(@layer 안)과 653행(밖).
#: 밖의 것이 이겨서 유틸리티를 덮는다. 처음에는 이것을 "실재하는 시각 버그" 라고 적고
#: /screener·/macro·/company 의 여백이 바뀔 일이라 사람이 정해야 한다고 미뤄 두었다.
#:
#: ★브라우저에서 재 보니 그 전제가 틀렸다★
#: 세 라우트에서 `.card-md` 렌더 개수는 **0** 이다. 이 22건이 있는 네 파일
#: (MacroRadar · narrative/index · ScreenerPanel · StockDetail)은 배럴 재export 말고는
#: **아무도 import 하지 않는 도달 불가 코드**다. 즉 무시되는 8개 padding 은 사용자가
#: 볼 수 없는 자리에 있다 — 정적 분석상의 불일치이지 시각 버그가 아니다.
#:
#: ★그리고 지우면 오히려 살아 있는 화면이 깨진다★
#: `.card-md` 가 실제로 렌더되는 곳은 /derivatives(1개)와 /dev/ui(3개)이고, 거기서는
#: 경쟁하는 padding 유틸리티가 없다. 653행을 지우면 그 넷의 padding 이 14px → **0px** 로
#: 떨어진다(실측). 즉 653행은 중복이 아니라 **살아 있는 화면이 기대는 규칙**이다.
#:
