...
# ═══════════════════════════════════════════════════════════════════════════
# 4. ★음수 클램프 — P3 에서 검토했고 유지하기로 했다★
# ═══════════════════════════════════════════════════════════════════════════

def test_build_plan_still_clamps_negative_targets_to_zero():
    """`execution_plan.py:72-73` 의 `max(..., 0.0)` — **현재 동작을 고정**한다.

    ★P3 결론: 바꾸지 않는다.★ 이 테스트가 "P3 가 롱숏을 열 때 대화 상대가 된다" 고
    적어 뒀고, 그 대화의 결과가 이것이다.

    P3 는 롱숏을 **연구·백테스트 전용**으로 열었다. 실행 경로는 공매도를 표현할 수
    없고(차입 미연동 · KIS 주문 유형 없음 · 실행기가 미보유 매도 생략), 그래서 롱숏
    목표는 `compile_target` 에서 항상 `research_only` 가 된다. 즉 이 클램프에 음수가
    도달하는 경로 자체가 없다.

    도달할 수 없는데도 남겨 두는 이유는, 클램프가 **두 번째 방어선**이기 때문이다.
    누군가 게이트를 우회해 `build_plan` 을 직접 부르면, 여기서 0 으로 막히는 편이
    음수 수량으로 주문 diff 를 만드는 것보다 낫다. 아래 짝 테스트가 "첫 번째 방어선은
    게이트" 라는 것을 증명한다.
    """
    from src.engine.execution_plan import build_plan
    ...
    tgt = {o["stock_code"]: o["tgt_weight_pct"] for o in plan["orders"]}
    assert tgt["005930"] == pytest.approx(0.0), "P3 에서 유지하기로 한 두 번째 방어선이 사라졌다"
