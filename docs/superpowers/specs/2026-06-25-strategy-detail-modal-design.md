...
## 6. 정직한 한계
- ★개정★ 과거 성과 곡선 = **실제 동적 백테스트**(매월 `as_of`로 그 시점 데이터만 보고 전략 비중 재계산 →
  다음 달 실현수익 복리). 모멘텀 로테이션·타이밍 전환을 재현 — buy&hold 아님. 단순화: 월 리밸런스·수수료/슬리피지 미반영·
  월말 종가 기준(일중 무시) — 정밀 비용·체결은 백테스터 탭 이식 사용.
