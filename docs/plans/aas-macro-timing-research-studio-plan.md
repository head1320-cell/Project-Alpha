...
### Phase C — Tailwind ↔ 레거시 특정도: **리팩터링 대신 가드**

계획은 `globals.css` 의 미분류 규칙 2,448개를 `@layer` 로 감싸 유틸리티가 항상 이기게
만드는 것이었다. 실행하지 않았고, 그 판단이 이 절의 내용이다.

**내가 처음 센 "충돌 0건" 은 틀렸다.** 그 계수는 감사가 고른 12개 파일만 훑었는데, 그
목록 자체가 레거시 접두사 정규식(`as|tfm|brun|…`)으로 만들어져 `card-md` 같은 이름이
애초에 들어 있지 않았다. 저장소 전체를 훑자 **22건**이 나왔다.

**원인은 중복 정의 하나다.** `.card-md` 가 `globals.css` 에 두 번 있다 — 191행(`@layer`
안)과 653행(밖). 밖의 것이 이겨 `padding: 1rem` 이 유틸리티를 덮는다. 14건은 `p-4`
(=1rem)라 눈에 보이는 차이가 없지만, **8건(`p-3`×3 · `p-6` · `p-8` · `p-12`×3)은 작성자가
쓴 값이 조용히 무시되고 있다** — 실재하는 시각 버그이며 `/screener` · `/macro` ·
`/company` 에 있다.

**그래도 전면 `@layer` 화는 하지 않는다.** 22건은 한 원인에서 나오고 국소적이라 저장소
전체 캐스케이드를 바꿀 근거가 되지 못한다. 무엇보다 ADR 001 이 적어 둔 대로 이 스위트는
**픽셀을 보지 않아** 캐스케이드 회귀를 스스로 잡지 못한다 — 되돌릴 근거조차 자동으로 얻지
못하는 변경이다.

**8건도 지금 고치지 않는다.** 고치면 살아 있는 화면의 여백이 실제로 바뀐다. 3rem 을
의도했는지, 1rem 으로 몇 달을 살아 그것이 기준이 됐는지는 **눈으로 확인할 수 있는 사람**이
정할 판단이다. 조용히 바꾸는 대신 목록으로 남긴다.

**대신 남긴 것: `tests/test_css_specificity_guard.py`** (정적 가드, 선례
`test_no_order_executor_bypass.py`).

| 테스트 | 지키는 것 |
|---|---|
| 합성 충돌 탐지 | ★"0건" 은 탐지기가 고장 나도 나오는 값이다★ `.tfm-row flex gap-2` 를 넣어 실제로 잡는지 확인 |
| 거짓 양성 없음 | `.num` + `mt-2` 는 잡지 않는다 — 시끄러운 가드는 무시된다 |
| 신규 충돌 0건 | 기존 22건은 기준선, 새로 들어오면 실패 |
| 기준선 부패 방지 | 고쳐 놓고 목록을 안 지우면 기준선이 거짓이 된다 — 사라진 항목도 알려 준다 |

**게이트(실측):** ruff clean · pytest **1534 / 10 skipped**(+5) · tsc 0 · eslint 0 error.
