...
SUBSTITUTES: dict[str, list[str]] = {
    ...
    # ── Butler 미지원 팩터 → 의도가 가장 가까운 지원 팩터(대체 칩) ──
    "직원급여총액": ["판관비율"], "평균급여": ["판관비율"], "1인당매출액": ["자산회전율"],
    "1인당영업이익": ["영업이익률"], "매출원가율": ["매출총이익률"],
    "매출채권회전율": ["자산회전율"], "재고자산회전율": ["자산회전율"], "현금회전일수": ["현금전환율"],
    "POR": ["주가수익률(PER)"], "주당순자산증가율": ["BPS"],
    "주당배당금": ["배당수익률"], "배당시점배당수익률": ["배당수익률"], "FCF배당성향": ["배당성향"],
    "배당금총액증가율": ["배당성장률"], "투자자주가수익률": ["12-1 모멘텀"], "총수익률": ["12-1 모멘텀"],
    "외국인지분율": ["외국인순매수금액"], "공매도잔고비율": ["20일평균거래량대비거래량"],
    "주가변동률1년": ["12-1 모멘텀"], "거래대금증가율": ["거래대금"],
    "목표주가수익률": ["12-1 모멘텀"], "예상매출액증가율": ["매출성장률(YoY)"],
    "예상영업이익증가율": ["영업이익성장률(YoY)"], "매출액Surprise": ["매출성장률(YoY)"],
    "자본지출(CAPEX)": ["설비투자 집약도"],
}
...
UNSUPPORTED_REASONS: dict[str, str] = {
    ...
    **{t: REASON_CONSENSUS for t in (
        "목표주가수익률", "분석보고서수", "커버증권사수",
        "예상매출액증가율", "예상영업이익증가율", "매출액Surprise", "영업이익Surprise")},
