...
class CandlestickSchema(BaseModel):
    ...
    alias: str | None = Field(default=None, description="패턴 별칭 (참조용)")
...
class IndicatorSchema(BaseModel):
    ...
    alias: str | None = Field(default=None, description="지표 내부 키 (참조용, 불변)")
    name: str | None = Field(default=None, description="표시 이름 (UI·리포트용)")
...
class ConditionSchema(BaseModel):
    ...
    """
    operator: OperatorType | None = Field(default=None, description="비교 연산자")
    indicator: str = Field(default="", description="왼쪽 지표 alias 또는 가격")
    indicator_output: str = Field(default="value", description="왼쪽 지표 출력값")
    compare_to: str | None = Field(default=None, description="오른쪽 지표 alias")
    compare_output: str = Field(default="value", description="오른쪽 지표 출력값")
    compare_scalar: float | None = Field(default=None, description="오른쪽 지표 스칼라/오프셋")
    compare_operation: str | None = Field(default=None, description="연산 종류 (mul, div, add, sub)")
    value: float | None = Field(default=None, description="비교 상수")
    candlestick: str | None = Field(default=None, description="캔들스틱 패턴 alias")
    signal: str | None = Field(default=None, description="캔들스틱 신호 (bullish, bearish, detected)")

    @field_validator('operator', mode='before')
    @classmethod
    def normalize_operator(cls, v: Any) -> OperatorType | None:
        """다양한 입력 형식을 표준화
...
class RiskSchema(BaseModel):
    ...
    """
    stop_loss_pct: float | None = Field(default=None, ge=0, le=100, description="손절 %")
    take_profit_pct: float | None = Field(default=None, ge=0, le=100, description="익절 %")
    trailing_stop_pct: float | None = Field(default=None, ge=0, le=100, description="트레일링 스탑 %")
    max_position_size: float | None = Field(default=None, ge=0, le=1, description="최대 포지션 비율")
...
class StrategySchema(BaseModel):
    ...
    risk: RiskSchema | None = Field(default=None, description="리스크 관리")
    ...
    def auto_populate_candlesticks(self) -> StrategySchema:
        ...
        """
        from src.kis_candlestick import PATTERN_DETECTORS as CANDLESTICK_REGISTRY
    ...
    @staticmethod
    def _extract_pattern_id(alias: str, registry: dict) -> str | None:
    ...
    def get_indicator_by_alias(self, alias: str) -> IndicatorSchema | None:
        """별칭으로 지표 찾기"""
