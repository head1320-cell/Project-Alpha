...
class IndicatorConfig(BaseModel):
    ...
    alias: str | None = Field(default=None, description="지표 내부 키 (Python 식별자, 조건 참조용)")
    name: str | None = Field(default=None, description="사용자 표시 이름 (자유 입력, 리포트·UI 표시용)")
...
class ConditionConfig(BaseModel):
    """조건 설정"""
    indicator: str | None = Field(default=None, description="지표 별칭")
    operator: str | None = Field(default=None, description="연산자 (greater_than, less_than, cross_above, cross_below)")
    compare_to: Union[str, float, int] | None = Field(default=None, description="비교 대상 (지표 별칭 또는 숫자)")
    value: Union[str, float, int] | None = Field(default=None, description="비교 값 (숫자 또는 $param_name 참조)")
    output: str | None = Field(default=None, description="지표 출력값")
    compare_output: str | None = Field(default=None, description="비교 지표 출력값")
    compare_scalar: float | None = Field(default=None, description="비교 지표 스칼라/오프셋 (예: 0.9, 1000)")
    compare_operation: str | None = Field(default=None, description="연산 종류 (mul, div, add, sub)")
    # 캔들스틱 조건용 필드
    candlestick: str | None = Field(default=None, description="캔들스틱 패턴 별칭")
    signal: str | None = Field(default=None, description="캔들스틱 시그널 (bullish, bearish, detected)")
...
class RiskConfig(BaseModel):
    """리스크 관리 설정"""
    stop_loss: dict[str, Any] | None = Field(
        default=None,
        description="손절 설정 {enabled: bool, percent: float}"
    )
    take_profit: dict[str, Any] | None = Field(
        default=None,
        description="익절 설정 {enabled: bool, percent: float}"
    )
    trailing_stop: dict[str, Any] | None = Field(
        default=None,
...
class CandlestickConfig(BaseModel):
    ...
    alias: str | None = Field(default=None, description="패턴 별칭")
    ...
    def validate_param_references(self) -> StrategyConfig:
        ...
        for cond in self.entry.conditions:
            if cond.value is not None:
                check(cond.value, "entry.conditions[].value")
            if cond.compare_to is not None:
                check(cond.compare_to, "entry.conditions[].compare_to")

        for cond in self.exit.conditions:
            if cond.value is not None:
                check(cond.value, "exit.conditions[].value")
            if cond.compare_to is not None:
                check(cond.compare_to, "exit.conditions[].compare_to")
...
class KisStrategyFile(BaseModel):
    ...
    def to_strategy_definition(self):
        ...
        from src.kis_strategy_definition import StrategyDefinition
