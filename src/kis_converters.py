...
from src.kis_param_resolver import ParamResolver
...
def _convert_condition_from_preset(
    ...
    """
    from src.kis_condition import CompositeCondition
...
def _convert_risk_from_preset(risk) -> RiskSchema | None:
    """Python 프리셋 리스크 변환"""
    if risk is None:
        return None
...
def _convert_risk_from_yaml(risk) -> RiskSchema | None:
    """YAML 리스크 변환"""
    if risk is None:
        return None
...
def _normalize_operator(op: str) -> OperatorType:
    ...
    from src.kis_schema import OPERATOR_ALIASES
