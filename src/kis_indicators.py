...
def calc_daily_change(df: pd.DataFrame) -> float | None:
    """
    전일 대비 등락률 계산
...
def calc_strong_close_ratio(df: pd.DataFrame) -> float | None:
    """
...
def calc_high_since(df: pd.DataFrame, days: int) -> int | None:
    """
    N일간 최고가 조회
...
def calc_low_since(df: pd.DataFrame, days: int) -> int | None:
    """
    N일간 최저가 조회
...
def get_latest_close(df: pd.DataFrame) -> int | None:
    """
    최근 종가 조회
...
def get_prev_close(df: pd.DataFrame) -> int | None:
    """
    전일 종가 조회
