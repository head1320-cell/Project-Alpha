...
@app.post("/api/v1/data/ingest/{target}")
def ingest_trigger(target: str):
    ...
    import threading
    if target == "all":
        targets = list(_INGEST_TARGETS)
    elif target in _INGEST_TARGETS:
        targets = [target]
    else:
    ...
    return {"started": started, "running": dict(_INGEST_RUNNING),
...
try:
    ...
    from src.api.backtest_run_routes import router as backtest_run_router
    ...
    from src.api.experimental_routes import router as experimental_router
    ...
    from src.api.sleeve_routes import router as sleeve_router
    ...
    app.include_router(sleeve_router)
